...
export default function MyCollection() {
  ...
  const showList = date.map(goodsinfo =>( 
    <div>
        ...
        <ul>
          <li style={{ display: 'flex', borderBottom: '1px solid rgba(0,0,0,0.1)', width: 750, padding: 10, marginRight: 0 }}>
            <div style={{ display: 'flex', flex: 2 }}>
              ...
              <div>
                <p style={{ fontSize: 12 }}>{props.info.name}</p>
              </div>
            </div>
  ...
  const collectionList = collectionData.map(listinfo =>
    <CollectionList key={listinfo.id} info={listinfo} />
  )
  return (
    <Layout>
